...
mysql_prepare() ->
    ...
    mysql:prepare(sensor_all_props, <<"SELECT m.device, m.external_id, m.function, un.string_id AS unit, m.description, m.port FROM logger_meters m, unit un WHERE m.unit_id = un.id AND m.meter = ?">>),
    ...
    mysql:prepare(sensor_function, <<"SELECT function FROM logger_meters WHERE meter = ?">>),
    ...
    mysql:prepare(device_pending_sensors, <<"SELECT m.meter FROM logger_meters m WHERE m.device = ? AND m.pending_config = 1">>),
    ...
    mysql:prepare(sensor_config, <<"UPDATE logger_meters SET external_id = ?, description = ?, unit_id = ? WHERE meter = ?">>),
    mysql:prepare(sensor_function_update, <<"UPDATE logger_meters SET function = ? WHERE meter = ?">>),
    mysql:prepare(sensor_port_update, <<"UPDATE logger_meters SET port = ? WHERE meter = ?">>),
    ...
    mysql:prepare(sensor_insert, <<"INSERT INTO logger_meters (meter, device, created, access, type, external_id, function, description, port, value, factor, unit_id, price, latitude, longitude) SELECT ?, device, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 0.18, 49.444710, 7.769031 FROM logger_devices WHERE device = ?">>),
    mysql:prepare(sensor_config_update, <<"UPDATE logger_meters set pending_config = ? WHERE meter = ?">>),
    ...
    mysql:prepare(energy_sensor_move, <<"UPDATE logger_energy_meter SET meter = ? WHERE meter = ?">>),
